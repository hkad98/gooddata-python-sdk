...
LISTLLMPROVIDERMODELSREQUESTPROVIDERCONFIG_ONE_OF_SCHEMAS = ["AwsBedrockProviderConfig", "AzureFoundryProviderConfig", "OpenAIProviderConfig"]

class ListLlmProviderModelsRequestProviderConfig(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ListLlmProviderModelsRequestProviderConfig.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ListLlmProviderModelsRequestProviderConfig with oneOf schemas: AwsBedrockProviderConfig, AzureFoundryProviderConfig, OpenAIProviderConfig. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ListLlmProviderModelsRequestProviderConfig with oneOf schemas: AwsBedrockProviderConfig, AzureFoundryProviderConfig, OpenAIProviderConfig. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into ListLlmProviderModelsRequestProviderConfig"
            " with oneOf schemas: AwsBedrockProviderConfig, AzureFoundryProviderConfig, OpenAIProviderConfig"
        )
