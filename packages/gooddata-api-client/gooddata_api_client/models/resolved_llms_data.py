...
RESOLVEDLLMSDATA_ONE_OF_SCHEMAS = ["ResolvedLlmEndpoint", "ResolvedLlmProvider"]

class ResolvedLlmsData(BaseModel):
    """
    ResolvedLlmsData
    """
    # data type: ResolvedLlmEndpoint
    oneof_schema_1_validator: Optional[ResolvedLlmEndpoint] = None
    # data type: ResolvedLlmProvider
    oneof_schema_2_validator: Optional[ResolvedLlmProvider] = None
    actual_instance: Optional[Union[ResolvedLlmEndpoint, ResolvedLlmProvider]] = None
    one_of_schemas: Set[str] = { "ResolvedLlmEndpoint", "ResolvedLlmProvider" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ResolvedLlmsData.model_construct()
        error_messages = []
        match = 0
        # validate data type: ResolvedLlmEndpoint
        if not isinstance(v, ResolvedLlmEndpoint):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ResolvedLlmEndpoint`")
        else:
            match += 1
        # validate data type: ResolvedLlmProvider
        if not isinstance(v, ResolvedLlmProvider):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ResolvedLlmProvider`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ResolvedLlmsData with oneOf schemas: ResolvedLlmEndpoint, ResolvedLlmProvider. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ResolvedLlmsData with oneOf schemas: ResolvedLlmEndpoint, ResolvedLlmProvider. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into ResolvedLlmsData"
            " with oneOf schemas: ResolvedLlmEndpoint, ResolvedLlmProvider"
        )
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ResolvedLlmEndpoint, ResolvedLlmProvider]]:
