...
import json
import pprint
from pydantic import BaseModel, ConfigDict, Field, StrictInt, StrictStr, ValidationError, field_validator, model_serializer
...
class AacDashboardFilterFrom(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AacDashboardFilterFrom with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into AacDashboardFilterFrom"
            " with oneOf schemas: int, str"
        )
