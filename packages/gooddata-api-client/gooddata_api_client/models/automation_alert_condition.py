...
class AutomationAlertCondition(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AutomationAlertCondition with oneOf schemas: AnomalyDetectionWrapper, ComparisonWrapper, RangeWrapper, RelativeWrapper. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into AutomationAlertCondition"
            " with oneOf schemas: AnomalyDetectionWrapper, ComparisonWrapper, RangeWrapper, RelativeWrapper"
        )
