...
from gooddata_api_client.models.dashboard_arbitrary_attribute_filter import DashboardArbitraryAttributeFilter
...
DASHBOARDFILTER_ONE_OF_SCHEMAS = ["DashboardArbitraryAttributeFilter", "DashboardAttributeFilter", "DashboardDateFilter", "DashboardMatchAttributeFilter"]
...
class DashboardFilter(BaseModel):
    ...
    # data type: DashboardArbitraryAttributeFilter
    oneof_schema_3_validator: Optional[DashboardArbitraryAttributeFilter] = None
    # data type: DashboardMatchAttributeFilter
    oneof_schema_4_validator: Optional[DashboardMatchAttributeFilter] = None
    actual_instance: Optional[Union[DashboardArbitraryAttributeFilter, DashboardAttributeFilter, DashboardDateFilter, DashboardMatchAttributeFilter]] = None
    one_of_schemas: Set[str] = { "DashboardArbitraryAttributeFilter", "DashboardAttributeFilter", "DashboardDateFilter", "DashboardMatchAttributeFilter" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, DashboardDateFilter):
            ...
            match += 1
        # validate data type: DashboardArbitraryAttributeFilter
        if not isinstance(v, DashboardArbitraryAttributeFilter):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DashboardArbitraryAttributeFilter`")
        else:
            match += 1
        # validate data type: DashboardMatchAttributeFilter
        if not isinstance(v, DashboardMatchAttributeFilter):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DashboardMatchAttributeFilter`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in DashboardFilter with oneOf schemas: DashboardArbitraryAttributeFilter, DashboardAttributeFilter, DashboardDateFilter, DashboardMatchAttributeFilter. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in DashboardFilter with oneOf schemas: DashboardArbitraryAttributeFilter, DashboardAttributeFilter, DashboardDateFilter, DashboardMatchAttributeFilter. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into DashboardFilter"
            " with oneOf schemas: DashboardArbitraryAttributeFilter, DashboardAttributeFilter, DashboardDateFilter, DashboardMatchAttributeFilter"
        )
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DashboardArbitraryAttributeFilter, DashboardAttributeFilter, DashboardDateFilter, DashboardMatchAttributeFilter]]:
