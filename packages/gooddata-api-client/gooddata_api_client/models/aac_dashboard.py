...
class AacDashboard(BaseModel):
    ...
    """
    # data type: AacDashboardWithTabs
    oneof_schema_1_validator: Optional[AacDashboardWithTabs] = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: AacDashboardWithTabs
        if not isinstance(v, AacDashboardWithTabs):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AacDashboardWithTabs`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AacDashboard with oneOf schemas: AacDashboardWithTabs, AacDashboardWithoutTabs. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        for variant in variant_classes:
            try:
                ...
                continue
        raise ValueError(
            "No match found when deserializing the JSON string into AacDashboard"
            " with oneOf schemas: AacDashboardWithTabs, AacDashboardWithoutTabs"
        )
