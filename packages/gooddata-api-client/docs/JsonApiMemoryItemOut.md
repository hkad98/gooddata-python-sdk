# JsonApiMemoryItemOut

JSON:API representation of memoryItem entity.

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**attributes** | [**JsonApiMemoryItemOutAttributes**](JsonApiMemoryItemOutAttributes.md) |  | 
**id** | **str** | API identifier of an object | 
**meta** | [**JsonApiAggregatedFactOutMeta**](JsonApiAggregatedFactOutMeta.md) |  | [optional] 
**relationships** | [**JsonApiDashboardPluginOutRelationships**](JsonApiDashboardPluginOutRelationships.md) |  | [optional] 
**type** | **str** | Object type | 

## Example

```python
from gooddata_api_client.models.json_api_memory_item_out import JsonApiMemoryItemOut

# TODO update the JSON string below
json = "{}"
# create an instance of JsonApiMemoryItemOut from a JSON string
json_api_memory_item_out_instance = JsonApiMemoryItemOut.from_json(json)
# print the JSON string representation of the object
print(JsonApiMemoryItemOut.to_json())

# convert the object into a dict
json_api_memory_item_out_dict = json_api_memory_item_out_instance.to_dict()
# create an instance of JsonApiMemoryItemOut from a dict
json_api_memory_item_out_from_dict = JsonApiMemoryItemOut.from_dict(json_api_memory_item_out_dict)
```
[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


