...
------------ | ------------- | ------------- | -------------
**allowed_relationship_types** | [**List[AllowedRelationshipType]**](AllowedRelationshipType.md) | Filter relationships and search results based on allowed relationship type combinations. When specified, only relationships matching the allowed types are returned (e.g. for view-only users). | [optional] 
**include_hidden** | **bool** | If true, includes hidden objects in search and visualization building. If false (default), excludes objects where isHidden&#x3D;true. | [optional] [default to False]
...
**object_types** | **List[str]** | List of object types to filter the search and visualization building. If empty or null, all object types are considered. | [optional] 
