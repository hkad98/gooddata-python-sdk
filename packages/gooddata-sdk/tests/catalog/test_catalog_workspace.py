...
def test_workspace_list(test_config):
    ...
    workspaces_id = sorted([w.id for w in workspaces])
    ids.sort()
    assert ids == workspaces_id

    workspaces_name = sorted([w.name for w in workspaces])
    names.sort()
    assert names == workspaces_name

    workspaces_parent_l = [w.parent_id for w in workspaces]
...
def test_clone_workspace(test_config):
    ...
    try:
        ...
        # TODO: Leaving this for todo, _refresh_workspaces will mess up with other agg fact tests
        #  If you fix the file in _refresh_workspaces it will mess with all the other tests
