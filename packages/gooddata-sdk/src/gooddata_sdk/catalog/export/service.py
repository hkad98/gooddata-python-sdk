...
class ExportService(CatalogServiceBase):
    ...
    def _create_export(
        ...
        return response.export_result if hasattr(response, "export_result") else response["export_result"]
    ...
    def get_raw_export_bytes(
        ...
        return self._get_exported_content(
            workspace_id=workspace_id,
            export_id=export_id,
            get_func=self._actions_api.get_raw_export_without_preload_content,
            timeout=timeout,
            retry=retry,
            max_retry=max_retry,
        )
